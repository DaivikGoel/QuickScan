...
//        }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        viewController.session.run(configuration)
    ...
    override func viewDidLoad() {
        ...
        viewController.delegate = self
        viewController.scene = SCNScene()
        
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(didTap))
        self.viewController.addGestureRecognizer(tapGesture)
        
        let fingerGesture = ThresholdPanGestureRecognizer(target: self, action: #selector(didFingerPan))
        self.viewController.addGestureRecognizer(fingerGesture)
        
        let rotateGesture = ThresholdRotationGestureRecognizer(target: self, action: #selector(didRotate(_:)))
        self.viewController.addGestureRecognizer(rotateGesture)
        
        let longPressGesture = UILongPressGestureRecognizer(target: self, action: #selector(didLongPress))
        self.viewController.addGestureRecognizer(longPressGesture)
        
        let pinchGesture = ThresholdPinchGestureRecognizer(target: self, action: #selector(didPinch))
        self.viewController.addGestureRecognizer(pinchGesture)
...
//        setupNavigationBar()
    ...
    override func viewDidLayoutSubviews() {
        ...
        screenCenter = viewController.center
    }
    ...
    private func createVideo(imageArray:[[String:Any]], fps:Int, size:CGSize, completionHandler:@escaping(String?)->()) {
            ...
            while (currentFrame < imageArray.count) {
                ...
                if (self.videoInput?.isReadyForMoreMediaData)!  {
                    ...
                    let pixelBuffer = (imageArray[currentFrame])["image"] as! CVPixelBuffer
...
//                    FileManager.default.clearTempMemory();
    ...
    public func didUpdateAtTime(time: TimeInterval, snapshot: CVPixelBuffer) {
       if self.isRecording {
           if self.lastTime == 0 || (self.lastTime + 1/11) < time {
               DispatchQueue.main.async { [weak self] () -> Void in
                   ...
                   self!.pixelBufferAdaptor!.append(snapshot, withPresentationTime: currentframeTime - self!.firstTime);
//                   self!.snapshotArray.append([
//                       "image": snapshot,
//                       "time": CMTime(value: CMTimeValue((self?.viewController.session.currentFrame!.timestamp)! * Double(scale)), timescale: scale)
//                   ]);
               }
           }
       }
...
//                            startLimitedTrackingTimer()
    ...
    func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
        if let objectAnchor = anchor as? ARObjectAnchor {
            ...
            viewController.stopPlaneDetection()
        }
    }
...
//        }
    ...
    func boundingBoxPositionOrExtentChanged(_ notification: Notification) {
        guard let box = notification.object as? BoundingBox,
            let cameraPos = viewController.pointOfView?.simdWorldPosition else { return }
